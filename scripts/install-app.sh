...
BIN="$(pwd)/.build/release/KuroTools"
...
if pgrep -f "$BIN" >/dev/null; then
  echo "✓ KuroTools is running in the background (and will start at login)."
  echo "  Quit from the menu bar to stop it; ./scripts/uninstall-app.sh to remove autostart."
else
  echo "⚠ KuroTools didn't start. Run '$BIN' directly to see any error."
fi
